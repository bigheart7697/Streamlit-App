...
if __name__ == '__main__':
    ...
    st.markdown(' Then you should tune the learning parameters.'
                'Please follow the format given each text field. That is start the input with [, fill out the list of'
                ' parameters seperated by , (comma) and end it with ]. Even if you are inputting one parameter in each'
                ' field, you should begin with [ and end with ]. Otherwise, or you will face an error. When all the'
                ' inputs are ready click on \'Enable Fitting\' button, so that the model get trained. When the '
                'success message printed on the page, the model is ready.')
    ...
    st.markdown('First note that this page will not show anything if you have not run the model. This page contains'
                'charts related to churn rate based on different attributes of customers. Attributes like gender,'
    ...
    st.markdown('This page contains curves and charts related to the performance of the model. You can see how the'
                ...
                'in a table and plots a line chart based on the precision, recall and f1-score measures.')
