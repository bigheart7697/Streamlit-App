...
if __name__ == '__main__':
    ...
    st.markdown('Churn rate is the percentage of subscribers of a service that are leaving. Businesses use this rate'
                ...
                'category of its subscribers are leaving. It should be able to identify which part of the service is'
                ' not satisfying and be able to effectively predict which customers are going to leave.')
    ...
    st.markdown('On the run the model page, the user inputs the data file and can tune the parameters for the'
                ...
                'On page Model Info, user can see how does the model performed. Also, user can compare different models'
                'that had been trained. Finally, on page Model Feature Analysis, Shaply values for the model are '
                'presented. Based on the reason for leaving the business can be identified. For more information,'
                'please refer to the User Guide page.')
